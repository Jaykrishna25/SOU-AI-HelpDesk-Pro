...
interface Tkt { code: string; subject: string; status: string; }

export default function StudentDashboard() {
  const [tab, setTab] = useState("Dashboard");
  const [tickets, setTickets] = useState<Tkt[]>([
    { code: "TKT-2026-0042", subject: "Fee receipt not generated", status: "In Progress" },
    { code: "TKT-2026-0031", subject: "Revaluation of DBMS paper", status: "Resolved" },
  ]);
  const reopen = (code: string) => setTickets((ts) => ts.map((t) => t.code === code ? { ...t, status: "Reopened" } : t));
  ...
  const downloadReceipt = () => {
    ...
    <body>
    <div class="head"><h1>Silver Oak University</h1><p>Official Fee Receipt - SOU AI HelpDesk Pro</p></div>
    <table>
      <tr><th>Student Name</th><td>Navlani Jaykrishna Satishkumar</td></tr>
      <tr><th>Enrollment No.</th><td>SOU2023CSE69</td></tr>
      <tr><th>Program</th><td>B.Tech CSE - Semester 5</td></tr>
      <tr><th>Receipt No.</th><td>RC-2026-000069</td></tr>
      <tr><th>Date</th><td>${now}</td></tr>
      <tr><th>Total Fees</th><td>Rs 1,20,000</td></tr>
      <tr><th>Amount Paid</th><td>Rs 90,000</td></tr>
      <tr><th>Balance Due</th><td>Rs 30,000 (due 15 Aug 2026)</td></tr>
      <tr class="total"><th>Payment Mode</th><td>Online (TXN-SOU2023CSE69-5)</td></tr>
    </table>
    <div class="foot">This is a computer-generated receipt and does not require a signature.<br>Generated by SOU AI HelpDesk Pro.</div>
    </body></html>`;
    const blob = new Blob([html], { type: "text/html" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url; a.download = "SOU-Fee-Receipt.html";
  ...
  const statusColor = (s: string) =>
    s === "Resolved" ? "bg-emerald-500/20 text-emerald-300" : s === "Reopened" ? "bg-rose-500/20 text-rose-300" : "bg-brand/20 text-brand-light";
                ...
                {tickets.map((t) => (
                  <div key={t.code} className="glass px-4 py-3 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {t.status === "Resolved" && <button onClick={() => reopen(t.code)} className="text-xs px-3 py-1 rounded-full glass">Reopen</button>}
                    </div>
                  </div>
                ))}
              </div>
